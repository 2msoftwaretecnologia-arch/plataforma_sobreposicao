...
class PhytoecologyAreaImporter:
    ...
    def execute(self):
        reset_db(PhytoecologyArea)
        user = self._get_user()
        archive_path = get_file_management()
        if not archive_path or not archive_path.phytoecology_zip_file.path:
            raise ValueError("Nenhum arquivo de fitoecologia foi configurado.")
