...
class Command(BaseCommand):
    help = "Processa dados geoespaciais de ipuca em paralelo com particionamento e hash única."
    ...
    def process_partition(self, partition_df, user):
        ...
        for _, row in partition_df.iterrows():
            ...
            try:
                obj, created = Ipuca.objects.get_or_create(
                ...
                results.append(obj.geometry)
                
                if created:
                    GeometryProcessingService(Ipuca).process_instance(obj)
                    print(f"[OK] {obj.geometry}")
                else:
                    print(f"[SKIP] {obj.geometry} já existe")
    ...
    def handle(self, *args, **options):
        print("Iniciando processamento de Ipuca em threads...")
        ...
        if not archive_path:
            raise CommandError("Nenhum arquivo de Ipuca foi configurado.")
        
        if not archive_path.ipuca_zip_file.path:
            raise CommandError("Nenhum arquivo de Ipuca foi configurado.")
        
        df = gpd.read_file(archive_path.ipuca_zip_file.path)
